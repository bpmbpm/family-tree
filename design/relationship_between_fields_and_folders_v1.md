...
При этом нужно учесть логику формирования галерей person и family, например, у person нет id_personAll (добавил, но не используется) у всех остальных есть, нужно единое правило отбра в "Список привязанных папок к текущему id", как вариант по id_personAll. Сейчас логика обработки по кнопке foto_family разная из окон Основные свойства person \ family, см. id отображаемое в окне.
